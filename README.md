...
- 版本标签:`v1.4.10`(当前);历史版本 exe 请见 GitHub Releases
...
- **数据统计**:设置中可视化展示备忘概况(总备忘/待办/今日到期/已过期/今日完成/周期重复/回收站)
- **日历视图**:顶栏 🗓 图标进入,按月展示备忘安排(今天高亮、过期标红、有备忘日期显示圆点图标、右上角数量角标),点击日期查看当天备忘
- **搜索高亮**:搜索时匹配关键字黄色背景高亮;**Ctrl+F** 快捷聚焦搜索框
- **托盘气泡**:鼠标悬停托盘图标显示当前待办数
- **编辑备忘**:右键「编辑」或双击备忘行,修改标题/内容/时间/重复/优先级/提前提醒/置顶,内容区可「🖼 添加图片」并单独删除
- **数据导入**:设置中「合并导入 JSON」(重复跳过、新增追加)/「替换导入 JSON」(覆盖全部),配合导出实现多设备手动同步
- **最近提醒**:设置面板查看近期触发过的提醒记录(到期/提前)
- **云端同步(WebDAV)**:配置网盘地址/账号/密码,一键「上传备份」/「下载恢复」(下载前自动备份),支持测试连接
- **关闭行为**:关闭按钮(X)直接退出应用,不再驻留托盘(提醒仅在应用运行时生效)
- **数据安全**:JSON 本地存储,启动自动备份(保留 7 份),一键导出 JSON / CSV
- **主题样式**:深色/浅色/跟随系统 × 6 套主题色(薄荷绿/海洋蓝/紫罗兰/琥珀橙/玫瑰红/青蓝),12 种观感即时切换
...
- ✅ 托盘常驻、关窗不退出、开机自启(关闭时对 Windows 注册表启动项显式清理,带兜底)
- ✅ 回收站恢复、一键导出、启动自动备份
